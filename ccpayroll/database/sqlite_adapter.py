"""
SQLite database adapter for Creative Closets Payroll application

This module provides the original SQLite database connection functionality.
"""
...
@contextmanager
def get_db():
    """Context manager for getting SQLite database connection"""
    ...
    if not hasattr(db_local, 'connection'):
        ...
        current_app.logger.info(f"New SQLite DB connection created for thread {thread_id}")
    else:
    ...
    try:
        ...
        current_app.logger.error(f"SQLite database error in thread {thread_id}: {str(e)}")
        raise
    finally:
...
def close_db():
    ...
    if hasattr(db_local, 'connection'):
        ...
        current_app.logger.info(f"Closed SQLite DB connection for thread {thread_id}")
...
def monitor_db_connections():
    ...
    for thread_id, info in list(db_connections.items()):
        ...
        if idle_time > 300:  # 5 minutes idle
            current_app.logger.warning(f"Thread {thread_id} has idle SQLite DB connection for {idle_time:.1f} seconds")
        
        if age > 3600:  # 1 hour old
            current_app.logger.info(f"Thread {thread_id} has SQLite DB connection open for {age:.1f} seconds")
